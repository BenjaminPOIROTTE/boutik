...
?>
			<li class="nav-item dropdown">
				...
				<div class="dropdown-menu" aria-labelledby="navbarDropdown">
					<?= ($_SESSION["connexion"] == 5)
						? "<a class=\"dropdown-item\" href=\"index.php?action=admin\">Gestion des devis</a>"
						: "<a class=\"dropdown-item\" href=\"index.php?action=suiviClient\">Suivi des devis</a>"
					?>
				</div>
			</li>
			<?php
	...
	public function produit($lesCategories, $infoArticle, $lesPhotos, $message)
	{
		$this->entete($lesCategories);

		// Fiche du produit à construire
		echo "
		<h2>" . $infoArticle['designationProduit'] . "</h2>
		...
		foreach ($lesPhotos as $photo) {
			...
			echo "<div class=\"col\"><img src='" . $photo["url"] . "' width='200'></div>";
		}
		echo "<div class=\"col\">";

		if (isset($_SESSION["connexion"]) && isset($_SESSION["categuser"])) {

			if (isset($_SESSION["connexion"]) && isset($_SESSION["categuser"]) && $_SESSION["categuser"] == 1) {
			?>
				<form action="./index.php?action=devis&id=<?= $_GET["id"] ?>" method="POST">
					<h3>Demande de devis</h3>

					Description:<br />
					<textarea name="description" style="width: 100%;"></textarea><br /><br />

					<button class="btn btn-primary">Envoyer mon devis</button>
				</form>
			<?php
			}

			if (isset($_SESSION["connexion"]) && isset($_SESSION["categuser"]) && $_SESSION["categuser"] == 0) {
			?>
				<form action="./index.php?action=devis&id=<?= $_GET["id"] ?>" method="POST">
					<h3>Devis client</h3>
		...
		} else echo "<h4>Veuillez vous connecter pour demander un devis.</h4>";
		...
		foreach ($lesArticles as $ligne) {
			echo "
				<tr>
					...
					<td>" . $ligne['prixProduit'] * $ligne['quantite'] . "€</td>
					<td><button type='submit' class=\"btn btn-secondary btn-sm\" name='supprimer' value='" . $ligne['codeProduit'] . "'>Supprimer</button></td>
				</tr>
			";
		}
	...
	public function categorie($lesCategories, $lesArticles, $lesPhotos, $nomCategorie)
	{
		$this->entete($lesCategories);
		...
		foreach ($lesArticles as $leProduit) {
			echo "<div class=\"col\"><a href='index.php?action=produit&id=" . $leProduit['codeProduit'] . "'><p>" . $leProduit['designationProduit'] . "</p>";

			foreach ($lesPhotos as $photo) {
				if ($photo["id_produit"] == $leProduit['codeProduit']) {
					echo "<img src='" . $photo["url"] . "' width='150'></a>";
					break;
				}
			}

			echo "<p>Sur devis</p></div>";
		}
	...
	public function admin($lesCategories, $lesDevis)
	{
		$this->entete($lesCategories);
		?>
		<form action="./index.php?action=admin" method="post">
			<table class="table">
				...
				?>
					<tr>
						<td><?= $unDevis["nomClient"] . " " . $unDevis["prenomClient"] ?></td>
						<td><?= $unDevis["designationProduit"] ?></td>
						<td><?= $unDevis["description"] ?></td>
						<td><button formaction="./index.php?action=refuse&id=<?= $unDevis['idDevis'] ?>" name="refuse" value="<?= $unDevis["idDevis"] ?>">Refuser</button></td>
						<td><button formaction="./index.php?action=accept&id=<?= $unDevis['idDevis'] ?>" name="accept" value="<?= $unDevis["idDevis"] ?>">Proposer</button></td>
					</tr>
				<?php
				}
				?>
	...
	public function acceptDevis($lesCategories)
	{
		$this->entete($lesCategories);
	?>
		<form action="./index.php?action=acceptDevisAdmin&id=<?= $_GET['id'] ?>" method="post">
			<textarea name="commentaire" rows="5" cols="33" required></textarea>
			<input type="number" name="prixDevis" required>
	...
	public function suiviClient($lesCategories, $lesDevis)
	{
		$this->entete($lesCategories);
	?>
		<h4>Bienvenue sur la page de suivi de vos devis</h4>
		<form method="post">
			<?php
			if (!empty($lesDevis)) {
			?>
				<table class="table">
					<tr>
						<th>Pour</th>
						<th>Info</th>
						<th>Bouton accepter</th>
						<th>Bouton refuser</th>
					</tr>
					<?php
					foreach ($lesDevis as $unDevis) {
					?>
						<tr>
							...
							if ($unDevis["isAcceptClient"] == 1) {
							?>
								<td>L’administrateur a accepté<br />Commentaire: <?= $unDevis["commentaireAdmin"] ?><br />Prix: <?= $unDevis["prixAdmin"] ?></td>
								<td><button formaction='./index.php?action=refu	se&id=<?= $unDevis["idDevis"] ?>' name='refuse' value='<?= $unDevis["idDevis"] ?>'>Refuser</button></td>
								<td><button formaction='./index.php?action=acceptDevisClient&id=<?= $unDevis["idDevis"] ?>' name='accept' value='<?= $unDevis["idDevis"] ?>'>Accepter</button></td>
							<?php
							}
							?>
						</tr>
				<?php
					...
					echo "Vous n'avez pas de devis en attente.";
				}
				?>
				</table>
		</form>
