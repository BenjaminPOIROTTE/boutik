...
class controleur {
	...
	public function produit() {
		if(isset($_GET["id"])) {
			...
			$photosArticle = (new photo)->getPhotoProduit($_GET["id"]);
			...
			if(count($infosArticle) > 0) {
				...
				(new vue)->produit($lesCategories, $infosArticle, $photosArticle, $message);
			}
			else {
				(new vue)->erreur404($lesCategories);
			}
	...
	public function devis() {
		$cat = (new categorie)->getAll();
		(new devis)->setDevis($_SESSION["connexion"], $_GET["id"], $_POST["description"]);
		(new vue)->accueil($cat);
	}

	public function admin() {
		$lesDevis = (new devis)->getDevisAdmin();
		$cat = (new categorie)->getAll();
		...
		(new vue)->admin($cat, $lesDevis);
	}

	public function suiviClient(){
		$cat = (new categorie)->getAll();
		$lesDevis=(new devis)->viewDevisClient($_SESSION["connexion"]);		
		(new vue)->suiviClient($cat, $lesDevis);
	}

	public function refuse() {
		$cat = (new categorie)->getAll();

		if($_SESSION["connexion"] != 6) {
			(new devis)->deleteDevis($_GET['id']);
			$devis = (new devis)->viewDevisClient($_SESSION["connexion"]);
			(new vue)->suiviClient($cat, $devis);
			return;
		}
		else {
			(new devis)->refuseDevis($_GET['id']);
	...
	public function acceptDevis() {
		$cat = (new categorie)->getAll();
		$devis = (new devis)->acceptDevis($_GET['id']);
		(new vue)->acceptDevis();
	}

	public function acceptDevisAdmin() {
		(new devis)->acceptDevis($_GET['id'], $_POST["commentaire"], $_POST["prixDevis"]);
		$lesDevis = (new devis)->getDevisAdmin();
		$cat = (new categorie)->getAll();
	...
	public function categorie() {
		...
		$photosArticle = (new photo)->getPhotoProduitForCategories($_GET["id"]);

		(new vue)->categorie($lesCategories, $lesArticles, $photosArticle, $nomCategorie);
	}
	...
	public function acceptDevisClient()
	{
		$lesCategories = (new categorie)->getAll();
		(new devis)->acceptDevisClient($_GET['id']);
		$lesDevis = (new devis)->viewDevisClient($_SESSION["connexion"]);
		$cat = (new categorie)->getAll();

		(new vue)->suiviClient($lesCategories, $lesDevis);
	}
}